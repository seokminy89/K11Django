...
각 변수명은 해당 input태그의 name 속성값으로 사용된다.
...
class QuestionForm(forms.Form):
    ...
    select 태그를 표현한다.(1개 선택)
        ...
        initial : 해당 태그의 default값으로 사용한다. 주로 수정에서 
            활용하면 된다. 소스보기 해보면 blue에 selected 되어 있음.
    ...
    '''(멀티 선택)
    select태그를 표현하는것은 5번과 동일하지만 multiple 속성을 부여하여
    ...
    <div>
        <label for="id_form7_1">
            <input type="checkbox" name="form7" value="green" id="id_form7_1" checked>녹색
        </label>
    </div>
    '''
...
class WriteForm(forms.Form):
    '''
        ...
        입렵폼의 타이틀 부분을 한글로 처리하기 위해 label을 추가한다.
    '''
